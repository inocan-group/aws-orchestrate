...
describe('Idempotence', () => {
  it('not finalized and not task states name generated by state machine should not change unless one states before/after (if exist) change', () => {
    const foo1 = State(s => s.pass({ name: 'foo1' }))
    const foo2NotFinalized = State(s => s.pass())
    const foo3 = State(s => s.pass({ name: 'foo3' }))
    const foo4 = State(s => s.pass({ name: 'foo4' }))

    const stepFn = StateMachine('fooStateMachine', {
      stepFunction: StepFunction(foo1, foo2NotFinalized, foo3, foo4),
    }).toJSON()

    const stepFn2 = StateMachine('fooStateMachine', {
      stepFunction: StepFunction(foo1, foo2NotFinalized, foo4),
    }).toJSON()

  })
  it.todo('Using same not finalized state in different step function context (parallel, choice) should generate different name')
})
