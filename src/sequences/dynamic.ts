 ...
 */
export function dynamic<T = any>(function_: string, property?: keyof T): IOrchestratedDynamicProperty {
  return {
    type: "orchestrated-dynamic-property",
    lookup: `${function_}${property ? `.${property}` : ""}`,
  };
}

export function isDynamic(object: IDictionary) {
  return !!(object?.type === "orchestrated-dynamic-property" && object.lookup);
}
