import { AwsArnService } from "common-types";
...
export function extractService(
  arnPartial: string
): { pre: string; service: AwsArnService; post: string } {
  ...
  const [_, pre, service, post] = arnPartial.match(re) as [string, string, AwsArnService, string];
