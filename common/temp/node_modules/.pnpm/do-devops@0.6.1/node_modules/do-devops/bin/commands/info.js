...
async function handler(argv, opts) {
    ...
    let npm;
    try {
        npm = await shared_1.getPackageInfo();
    }
    catch (e) {
        // appears NOT to be a NPM package
    }
    const pkg = await shared_1.getPackageJson();
    const priorVersions = npm
        ? npm.versions
            ...
            .join(", ")
        : "";
    const gitLastCommit = await shared_1.getGitLastCommit();
    const branch = await shared_1.getCurrentGitBranch();
    const localFilesChanged = (await async_shelljs_1.asyncExec("git diff --name-only", {
        silent: true,
    })).split("\n").length;
    ...
    const npmInfo = [
        ...
        [
            ...
            npm
                ...
                : chalk_1.default `Locally in {italic package.json}, the version is ${chalk_1.default.bold.green(pkg.version)} but this is {italic not} an npm package.`,
        ],
    ...
    const data = [
        ["Desc ", exports.description ? pkg.description : chalk_1.default.bold.italic("no description provided!")],
        [
            "NPM",
            npmInfo
