...
async function createFunctionDictionary(rootPath) {
 ...
 */
async function writeServerlessFunctionExports(basePath = undefined, output = undefined) {
 ...
 * definition.
 */
async function validateExports(fnDefns) {
    const valid = [];
    const invalid = [];
    for (const fn of fnDefns) {
        try {
            const imp = await Promise.resolve().then(() => __importStar(require(fn)));
