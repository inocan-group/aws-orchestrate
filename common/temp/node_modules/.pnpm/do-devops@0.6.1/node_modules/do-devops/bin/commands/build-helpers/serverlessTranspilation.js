 ...
 */
async function serverlessTranspilation(c) {
    const { argv, opts, config, tooling } = c;
    let fns;
    const validity = await filterOutInvalidFunction(argv);
    if (opts.interactive || argv.length > 0) {
        fns = await index_1.askForFunctions(validity.invalid.length > 0
            ? chalk_1.default `Some of the functions you stated were invalid [ {grey ${validity.invalid.join(", ")}} ].\nChoose the functions from the list below:`
...
async function filterOutInvalidFunction(fns) {
    const validFns = Object.keys(await index_2.getLocalServerlessFunctionsFromServerlessYaml());
    const results = {
        valid: [],
        invalid: [],
